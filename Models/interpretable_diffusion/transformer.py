...
class GatedLocalCNN(nn.Module):
    def __init__(self, channels, kernel_size=5):
        super().__init__()
        self.local_conv = nn.Conv1d(channels, channels, kernel_size=kernel_size, padding=kernel_size // 2)
    ...
    def forward(self, x):  # x: [B, T, C]
        ...
        gated_feature = self.local_conv(x) * self.gate(x)
        return gated_feature.transpose(1, 2)  # [B, T, C]

class GraphConvolution(nn.Module):
    """基于电极位置的空间关系建模"""

    def __init__(self, in_features, out_features, n_channel):
        super().__init__()
        ...
        if n_channel == 23:
            channel_names = [
                ...
                'T3', 'C3', 'CZ', 'C4', 'T4',
                'T5', 'P3', 'PZ', 'P4', 'T6',
                'O1', 'O2', 'AF3', 'AF4', 'CP3', 'CP4'
            ]
            coords = {
                'FP1': (1, 5), 'FP2': (5, 5), 'F7': (0, 4), 'F3': (2, 4), 'FZ': (3, 4), 'F4': (4, 4), 'F8': (6, 4),
                'T3': (0, 3), 'C3': (2, 3), 'CZ': (3, 3), 'C4': (4, 3), 'T4': (6, 3),
                'T5': (0, 2), 'P3': (2, 2), 'PZ': (3, 2), 'P4': (4, 2), 'T6': (6, 2),
                'O1': (2, 1), 'O2': (4, 1), 'AF3': (2, 5), 'AF4': (4, 5), 'CP3': (2, 2.5), 'CP4': (4, 2.5)
            }

            adj = np.zeros((n_channel, n_channel))
            for i in range(n_channel):
                for j in range(n_channel):
                    if i == j: continue
                    xi, yi = coords[channel_names[i]]
                    xj, yj = coords[channel_names[j]]
        ...
        self.linear = nn.Linear(n_channel, out_features)  # 修正
        self.activation = nn.GELU()

    def forward(self, x):
        # x: [B, T, C] → [B, C, T]
        x = x.transpose(1, 2)
        x = torch.matmul(self.adj_matrix, x)  # [B, C, T]
        x = x.transpose(1, 2)  # → [B, T, C]
        x = self.linear(x)     # → [B, T, out_features]
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.local_detector = GatedLocalCNN(n_embd)
    ...
    def forward(self, x, encoder_output, timestep, mask=None, label_emb=None):
        ...
        # Add local anomaly detector
        x = x + self.local_detector(x)
...
class Transformer(nn.Module):
    def __init__(
        ...
        super().__init__()
        # 添加图卷积层
        self.gcn = GraphConvolution(n_feat, n_feat, n_feat)
